...
from config import (
    LIMITS,
    READ_LOOP_MODEL,
    READ_LOOP_PROMPT_CACHE_KEY,
    create_read_loop_model,
)
...
# Compact once the raw observations genuinely threaten the context window.
#
# This was 24_000 chars — about 6k tokens, or six trimmed observations, so it
# fired around step six of any real investigation and then kept firing. That
# is the wrong trade, because compaction is not free: it rewrites ToolMessages
...
# The context block is rebuilt every step and sits after the cached prefix, so
# it is the one part of every request billed at the full input rate — its size
# is multiplied by the step count. While stepping, the model needs the recent
# findings to know where it is; the whole ledger is only needed once, when it
# writes the answer, so only that call pays for the whole ledger.
STEP_EVIDENCE_LIMIT = 12
STEP_OBSERVED_FILE_LIMIT = 25
# --------------------------------------------------------------------------
...
# A refused call (exact repeat or redundant read) tells the model precisely
# what it already has and what to do instead. A model that keeps calling
# anyway is not going to be talked out of it by a clearer sentence — observed
# directly, gpt-5.4-mini retried one already-refused read twelve times in a
# single request. Past this many refusals in one request, stop offering
# advice and force the answer instead of paying for the thirteenth retry.
# Each refusal still costs a full round trip — the whole context is re-sent to
# be told "no" — so the ceiling is deliberately low. Three is enough slack for
# a model that misjudged an overlap once; five was five re-sends of everything.
MAX_THRASH_REFUSALS = 3
...
class Orchestrator:
    def __init__(
        ...
        *,
        ...
        prompt_cache_key: str = READ_LOOP_PROMPT_CACHE_KEY,
    ):
    ...
    def model(self):
        if self._model is None:
            self._model = create_read_loop_model(
                cache_key=self.prompt_cache_key
            ).bind_tools(self._tool_schemas())
        return self._model
    ...
    def _build_messages(self, *, final_instruction: str | None = None) -> list[BaseMessage]:
        ...
        # Finalisation is one call and needs every finding it might cite.
        # Stepping happens `max_steps` times and only needs recent bearings —
        # and it is the only one of the two that is still deciding what to
        # read, so it is the only one the observed-range list can help.
        finalising = final_instruction is not None
        context = prompts.build_context_message(
            ...
            evidence=self.ledger.render_for_prompt(
                limit=None if finalising else STEP_EVIDENCE_LIMIT
            ),
            observed=(
                "" if finalising
                else self.ledger.render_observed_files(limit=STEP_OBSERVED_FILE_LIMIT)
            ),
    ...
    def _loop(self) -> tuple[str, str]:
        while self._steps_used < self.max_steps:
            ...
            if self._thrash_refusals >= MAX_THRASH_REFUSALS:
                log.info("thrash threshold reached", extra={"agent": self.name})
                return self._finalise(prompts.THRASH_INSTRUCTION), "thrash_detected"
    ...
    def _redundant_read(self, arguments: dict[str, Any]) -> str | None:
        ...
        try:
            ...
            # Must mirror `read_tools.read`'s own default, or an omitted limit
            # is judged against a window the tool will never actually return.
            requested = int(arguments.get("limit") or LIMITS.default_read_lines)
    ...
    def _execute_tool_call(self, call: dict[str, Any]) -> None:
        ...
        log.info(
            ...
            extra={
                ...
                "ok": result.ok,
                "repeated": bool(result.metadata.get("repeated")),
    ...
    def _finalise(self, instruction: str) -> str:
        ...
        try:
            # Finalisation must produce prose, not another tool call — but it
            # must not do that by dropping the tools. Tool schemas are part of
            # the cached prefix, so an unbound client makes the largest request
            # of the whole run a 100% cache miss, at 10x the cached rate.
            # `tool_choice="none"` forbids calling them while keeping the
            # prefix byte-identical to every step that came before it.
            final_model = create_read_loop_model(
                cache_key=self.prompt_cache_key
            ).bind_tools(self._tool_schemas(), tool_choice="none")
            aggregate = None
